local Components = {
  ...
  { label = "Lunettes 1", name = "glasses_1", value = -1, min = -1, zoomOffset = 0.6, camOffset = 0.65},
...
function LoadDefaultModel(malePed, cb)
  ...
  Citizen.CreateThread(function()
    ...
    TriggerEvent('caruiskinchanger:modelLoaded')
...
AddEventHandler('caruiskinchanger:loadDefaultModel', function(loadMale, cb)
  LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('caruiskinchanger:getData', function(cb)
...
AddEventHandler('caruiskinchanger:change', function(key, val)

  Character[key] = val

  if key == 'sex' then
    TriggerEvent('caruiskinchanger:loadSkin', Character)
  else
    ApplySkin(Character)
  end
...
AddEventHandler('caruiskinchanger:getSkin', function(cb)
  cb(Character)
end)
...
RegisterNetEvent('caruiskinchanger:loadSkin')
AddEventHandler('caruiskinchanger:loadSkin', function(skin, cb)
  ...
  if skin['sex'] ~= LastSex then
    ...
    if skin['sex'] == 0 then
      TriggerEvent('caruiskinchanger:loadDefaultModel', true, cb)
    else
      TriggerEvent('caruiskinchanger:loadDefaultModel', false, cb)
    end
  ...
  Citizen.CreateThread(function()
    -- Vérifier si le modèle est valide avant de demander son chargement
    if not IsModelInCdimage(characterModel) or not IsModelValid(characterModel) then
        return
    end

    -- Demander le modèle et attendre qu'il soit chargé
    RequestModel(characterModel)

    -- Attente jusqu'à ce que le modèle soit chargé
    while not HasModelLoaded(characterModel) do
        Citizen.Wait(10)  -- Augmenter l'attente à 10ms pour réduire l'impact sur les performances
    end

    -- Appliquer le modèle au joueur si tout est valide
    SetPlayerModel(PlayerId(), characterModel)
    SetPedDefaultComponentVariation(playerPed)

    -- Libérer le modèle une fois qu'il est utilisé
    SetModelAsNoLongerNeeded(characterModel)

    -- Déclencher l'événement pour indiquer que le modèle a été chargé
    TriggerEvent('caruiskinchanger:modelLoaded')
end)
...
RegisterNetEvent('caruiskinchanger:loadClothes')
AddEventHandler('caruiskinchanger:loadClothes', function(playerSkin, clothesSkin)
  ...
  if playerSkin['sex'] ~= LastSex then
    ...
    if playerSkin['sex'] == 0 then
      TriggerEvent('caruiskinchanger:loadDefaultModel', true)
    else
      TriggerEvent('caruiskinchanger:loadDefaultModel', false)
    end
  ...
  Citizen.CreateThread(function()
    ...
    -- Attente initiale pour laisser le temps au modèle de commencer à charger
    Citizen.Wait(100)
    
    -- Attendre que le modèle soit chargé avec un délai de 10ms au lieu de 0ms
    while not HasModelLoaded(characterModel) do
        Citizen.Wait(20) -- 10ms au lieu de 0ms pour réduire l'utilisation CPU
    end
    
    -- Vérification et application du modèle
    if IsModelInCdimage(characterModel) and IsModelValid(characterModel) then
        SetPlayerModel(playerPed, characterModel)
        SetPedDefaultComponentVariation(playerPed)
    end
    
    -- Libération de la mémoire et notification
    SetModelAsNoLongerNeeded(characterModel)
    TriggerEvent('caruiskinchanger:modelLoaded')
end)
...
--[[if ESX.PlayerData.skin then
  TriggerEvent('caruicaruiskinchanger:loadSkin', ESX.PlayerData.skin)
else
  TriggerEvent('caruicaruiskinchanger:loadSkin', {sex = 0})
end]]
end)