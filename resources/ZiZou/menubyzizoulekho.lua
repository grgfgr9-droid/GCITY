...
local spriteW, spriteH = .225, .080
local PMenu = {}
local parentSliderSize = .25

local DefautColor = { 255, 255, 255, 155 }

-- Hello There, I know you're trying to dump my menus, stop that's not verry verry verry good
...
function PMenu:resetMenu()
	...
	self.Base = {
		...
		Title = GCore and GCore.user and GCore.user.name or "Menu",
...
function UpdateSlide(btnname)
	local _btnname = btnname
	if PMenu.IsVisible and not PMenu.Invisible then
		Citizen.CreateThread(function()
		for k, v in pairs(PMenu:GetButtons()) do
			if v.name == _btnname then
				if v.slidenum then
				v.slidenum = 1
				end
				break
...
function PMenu:CreateMenu(tableMenu, tempData)
	--if not PassJoin then return end
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	local tableColor, add, currentMenuData = boolSelected and ({ DefautColor[1], DefautColor[2], DefautColor[3], 200 }) or ({ 30, 30, 30, 130 }), .0, self.Menu[self.Data.currentMenu]
	...
	local stringPrefix = (((button.r and (((GCore and (GCore.jobRank < button.r or GCore.copRank < button.r)) or (button.rfunc and not button.rfunc())) and "~r~" or "")) or "") .. (self.Events["setPrefix"] and self.Events["setPrefix"](button, self.Data) or "")) or ""
	DrawText2(0, stringPrefix .. (button.name or ""), .275, intX - intW / 2 + .005, intY - intH / 2 + .0025, tableColor)
	...
	if unkCheckbox ~= nil and (button.checkbox ~= nil or currentMenuData and currentMenuData.checkbox ~= nil) then
		local bool = unkCheckbox ~= nil and (type(unkCheckbox) == "function" and unkCheckbox(ESX.PlayerData.cache.playerped, button, self.Base.currentMenu, self)) or unkCheckbox
	...
	local textBonus = (self.Events["setBonus"] and self.Events["setBonus"](button, self.Data.currentMenu, self)) or (button.amount and button.amount)
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		drawSprite(parentHeader, childHeader, self.Width - spriteW / 2, self.Height - intH / 2, spriteW, intH, .0, ZiZouConfig.Pmenu.Couleur.r, ZiZouConfig.Pmenu.Couleur.g, ZiZouConfig.Pmenu.Couleur.b, ZiZouConfig.Pmenu.Couleur.o)
		...
		if not self.Base.CustomHeader then
			DrawText2(1, self.Base.Title, .6, self.Width  - spriteW / 2, self.Height - intH / 2 + .0125, color_white, false, 0)
		end
	end
...
AddEventHandler("pmenu:ExternalOpen", function(menu)
CreateMenu(menu)
end)

AddEventHandler("pmenu:ExternalClose", function()
CloseMenu(true)
end)