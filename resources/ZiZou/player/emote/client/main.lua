...
function startScenario(anim)
        ...
        TaskStartScenarioAtPosition(PlayerPedId(), anim, PlayerPos['x'], PlayerPos['y'], PlayerPos['z'], GetEntityHeading(PlayerPedId()), 0, 1, false)
end 
...
RegisterCommand('annulerEmote', function()
    Ptfx = false
		...
		for k, v in pairs(vCat.simple) do
			if args[1] == v.command then
				if v.name == "Faite pleuvoir" or v.name == "Spray au champagne" or v.name == "Caméra" then 
					Ptfx = true	
					DestroyAllProps()
					ESX.ShowNotification('Appuyer  ~p~ G ~s~sur utiliser l\'attaque spécial.')
...
function AnimationMenu()
    local AnimMenu = {
        ...
        Data = {
            currentMenu = "Actions disponibles"
        },
        Events = {
            onSelected = function(self, _, btn)
                if btn.name == "Animations de marche" then
                    OpenMenu("Marche")
                elseif btn.name == "Salutations" then
                    OpenMenu("Salutations")
                elseif btn.name == "Annuler l'emote" then
                    ExecuteCommand('annulerEmote')
                elseif btn.name == "Frapper" then
                    OpenMenu("Frapper")
                elseif btn.name == "Sexe" then
                    OpenMenu("Sexe")
                elseif btn.name == "Sits" then
                    OpenMenu("Sits")
                elseif btn.name == "Sport" then
                    OpenMenu("Sport")
                elseif btn.name == "Gang" then
                    OpenMenu("Gang")
                elseif btn.name == "Poses" then
                    OpenMenu("Poses")
                elseif btn.name == "Actions" then
                    OpenMenu("Actions")
                elseif btn.name == "Standard" then
                    OpenMenu("Marche_Standard")
                elseif btn.name == "Styles spécifiques" then
                    OpenMenu("Marche_Styles")
                elseif btn.name == "Personnages" then
                    OpenMenu("Marche_Personnages")
                elseif btn.name == "États" then
                    OpenMenu("Marche_Etats")
                elseif btn.name == "Métiers" then
                    OpenMenu("Marche_Metiers")


                elseif btn.name == "Bras Croisé" then
                    OpenMenu("Marche_croise")
                elseif btn.name == "Inactif" then
                    OpenMenu("Marche_Inactif")
                elseif btn.name == "Attendre" then
                    OpenMenu("Marche_Attendre")
                elseif btn.name == "Penser" then
                    OpenMenu("Marche_Penser")
                elseif btn.name == "Position" then
                    OpenMenu("Marche_Position")

                elseif btn.name == "Jogging" then
                    OpenMenu("Marche_Jogging")
                elseif btn.name == "Etirement/Sport" then
                    OpenMenu("Marche_Etirement/Sport")
                elseif btn.name == "Golf" then
                    OpenMenu("Marche_Golf")
                elseif btn.name == "Karaté" then
                    OpenMenu("Marche_Karaté")
                elseif btn.name == "Boxe" then
                    OpenMenu("Marche_Boxe")


                elseif btn.name == "S'asseoir" then
                    OpenMenu("Marche_sits")
                elseif btn.name == "Tomber" then
                    OpenMenu("Marche_Tomber")

                elseif btn.name == "Autres" then
                    OpenMenu("Marche_Autres")
                elseif btn.action == "walk" then
                    local walkStyle = btn.value
                ...
                elseif btn.action == "salute" then
                    local dict = btn.dict
                    local anim = btn.anim
                    local loop = btn.loop or 48
                    ...
                    if loop == 51 then -- Boucle continue
                        TaskPlayAnim(PlayerPedId(), dict, anim, 8.0, -8.0, -1, 1, 0, false, false, false)
                    else -- Animation temporaire
                        TaskPlayAnim(PlayerPedId(), dict, anim, 8.0, -8.0, -1, loop, 0, false, false, false)
                    end
                end -- <- Assurez-vous de bien fermer le dernier bloc `end`
            end
        },
        ...
        Menu = {
            ["Actions disponibles"] = {
                b = {
                    {name = "Annuler l'emote", ask = "→", askX = true},
                    {name = "Animations de marche", ask = "→", askX = true},
                    {name = "Salutations", ask = "→", askX = true},
                    {name = "Frapper", ask = "→", askX = true},
                    {name = "Sexe", ask = "→", askX = true},
                    {name = "Poses", ask = "→", askX = true},
                    {name = "Sits", ask = "→", askX = true},
                    {name = "Gang", ask = "→", askX = true},
                    {name = "Sport", ask = "→", askX = true},
                  --  {name = "Actions", ask = "→", askX = true}
                    -- Vous pouvez ajouter d'autres catégories d'animations ici
                }
            },
            ["Marche"] = {
                b = {
                    {name = "Standard", ask = "→", askX = true},
                    {name = "Styles spécifiques", ask = "→", askX = true},
                    {name = "Personnages", ask = "→", askX = true},
                    {name = "États", ask = "→", askX = true},
            ...
            ["Sport"] = {
                b = {
                    {name = "Boxe", ask = "→", askX = true},
                    {name = "Karaté", ask = "→", askX = true},
                    {name = "Golf", ask = "→", askX = true},
                    {name = "Jogging", ask = "→", askX = true},
                    {name = "Etirement/Sport", ask = "→", askX = true},
                }
            },
            ["Sits"] = {
                b = {
                    {name = "S'asseoir", ask = "→", askX = true},
                    {name = "Tomber", ask = "→", askX = true},
                }
            },
            ["Poses"] = {
                b = {
                    {name = "Bras Croisé", ask = "→", askX = true},
                    {name = "Inactif", ask = "→", askX = true},
                    {name = "Attendre", ask = "→", askX = true},
                    {name = "Penser", ask = "→", askX = true},
    ...
    AnimMenu.Menu["Marche_Standard"] = { b = {} }
    AnimMenu.Menu["Marche_Styles"] = { b = {} }
    AnimMenu.Menu["Marche_Personnages"] = { b = {} }
    AnimMenu.Menu["Marche_Etats"] = { b = {} }
    AnimMenu.Menu["Marche_Metiers"] = { b = {} }
    AnimMenu.Menu["Marche_Autres"] = { b = {} }

    AnimMenu.Menu["Marche_croise"] = { b = {} } 
    AnimMenu.Menu["Marche_Position"] = { b = {} }
    AnimMenu.Menu["Marche_Penser"] = { b = {} }
    AnimMenu.Menu["Marche_Attendre"] = { b = {} }
    AnimMenu.Menu["Marche_Inactif"] = { b = {} }

    AnimMenu.Menu["Marche_Etirement/Sport"] = { b = {} } 
    AnimMenu.Menu["Marche_Jogging"] = { b = {} }
    AnimMenu.Menu["Marche_Golf"] = { b = {} }
    AnimMenu.Menu["Marche_Karaté"] = { b = {} }
    AnimMenu.Menu["Marche_Boxe"] = { b = {} }


    AnimMenu.Menu["Marche_sits"] = { b = {} }
    AnimMenu.Menu["Marche_Tomber"] = { b = {} }
    for k, v in pairs(Walk) do
        local label = v.label
        local value = v.value
        local button = {name = label, value = value, action = "walk"}
        
        if label:find("defaut") or label:find("Femme") or label:find("Homme") and not (label:find("Gangster") or label:find("Difficile")) then
            table.insert(AnimMenu.Menu["Marche_Standard"].b, button)
        elseif label:find("Franklin") or label:find("Trevor") or label:find("Michael") or label:find("Lester") or label:find("Lemar") then
            table.insert(AnimMenu.Menu["Marche_Personnages"].b, button)
        elseif label:find("Bourre") or label:find("Triste") or label:find("Sexy") or label:find("Appeure") or label:find("Confiant") or label:find("Courageux") or label:find("Decontracte") or label:find("Insolent") then
            table.insert(AnimMenu.Menu["Marche_Etats"].b, button)
        elseif label:find("Flic") or label:find("Gardien") or label:find("prison") or label:find("Poubelle") then
            table.insert(AnimMenu.Menu["Marche_Metiers"].b, button)
        elseif label:find("Gangster") or label:find("Chic") or label:find("Hipster") or label:find("Clochard") or label:find("Arrogant") or label:find("Talons") or label:find("Argent") or label:find("Muscle") or label:find("Alien") or label:find("Blindé") then
            table.insert(AnimMenu.Menu["Marche_Styles"].b, button)
        else
            table.insert(AnimMenu.Menu["Marche_Autres"].b, button)
        end
    end

    if Salue and Salue[1] and Salue[1].salue then
        for _, v in pairs(Salue[1].salue) do
            table.insert(AnimMenu.Menu["Salutations"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Frapper and Frapper[1] and Frapper[1].frapper then
        for _, v in pairs(Frapper[1].frapper) do
            table.insert(AnimMenu.Menu["Frapper"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Sexe  then
        for _, v in pairs(Sexe) do
            table.insert(AnimMenu.Menu["Sexe"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Sexe  then
        for _, v in pairs(Gang[1].gang) do
            table.insert(AnimMenu.Menu["Gang"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Poses[1].brascroise) do
            table.insert(AnimMenu.Menu["Marche_croise"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Poses[1].inactif) do
            table.insert(AnimMenu.Menu["Marche_Inactif"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Poses[1].wait) do
            table.insert(AnimMenu.Menu["Marche_Attendre"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Poses[1].penser) do
            table.insert(AnimMenu.Menu["Marche_Penser"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Poses[1].ppp) do
            table.insert(AnimMenu.Menu["Marche_Position"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sits[1].sits) do
            table.insert(AnimMenu.Menu["Marche_sits"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sits[1].tomber) do
            table.insert(AnimMenu.Menu["Marche_Tomber"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sport[1].box) do
            table.insert(AnimMenu.Menu["Marche_Boxe"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sport[1].karate) do
            table.insert(AnimMenu.Menu["Marche_Karaté"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sport[1].jog) do
            table.insert(AnimMenu.Menu["Marche_Jogging"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sport[1].etirement) do
            table.insert(AnimMenu.Menu["Marche_Etirement/Sport"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Poses then
        for _, v in pairs(Sport[1].golf) do
            table.insert(AnimMenu.Menu["Marche_Golf"].b, {
                name = v.name, 
                dict = v.value, 
                anim = v.anim, 
                loop = v.loop, 
    ...
    if Actions and Actions[1] and Actions[1].actions then
        for _, v in pairs(Actions[1].actions) do
            table.insert(AnimMenu.Menu["Actions"].b, {
                name = v.name, 
                scenario = v.scenario,
                dict = v.value, 
                anim = v.anim, 
