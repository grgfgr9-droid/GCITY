...
function AttempLogDeath()
	...
	local PedKiller = GetPedSourceOfDeath(ESX.PlayerData.cache.playerped)
			local killername = GetPlayerName(PedKiller)
			DeathCauseHash = GetPedCauseOfDeath(ESX.PlayerData.cache.playerped)
...
--[[Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(500)
		if IsEntityDead(ESX.PlayerData.cache.playerped) then
			Citizen.Wait(0)
			local PedKiller = GetPedSourceOfDeath(ESX.PlayerData.cache.playerped)
			local killername = GetPlayerName(PedKiller)
			DeathCauseHash = GetPedCauseOfDeath(ESX.PlayerData.cache.playerped)
		...
		while IsEntityDead(ESX.PlayerData.cache.playerped) do
			Citizen.Wait(0)
		end
	end
end)]]
