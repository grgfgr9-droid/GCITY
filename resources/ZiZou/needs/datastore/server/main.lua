...
MySQL.ready(function()
	local result = {{name = "property", label = "Propriété", shared = 0}}
	for k, v in pairs(ESX.Jobs) do 
		table.insert(result, {name = "society_" .. v.name, label = v.label, shared = 1})
	end
...
AddEventHandler('datastore:getDataStore', function(name, owner, cb)
	cb(GetDataStore(name, owner))
end)

AddEventHandler('datastore:getDataStoreOwners', function(name, cb)
	cb(GetDataStoreOwners(name))
end)

AddEventHandler('datastore:getSharedDataStore', function(name, cb)
