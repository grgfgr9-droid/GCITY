...
function ESX.ShowNotification(msg, flash, saveToBrief, hudColorIndex, title, subject, icon)
    if title == nil then title = ZiZouConfig.ServerName end
    if subject == nil then subject = "Notification" end
    if icon == nil then icon = "message" end

    exports.ZiZouNotif:Send(msg, nil, nil, true, nil, title, subject, icon)
end 
...
function ESX.ShowAdvancedNotification(title, subject, msg, banner, timeout, icon)
    ...
    exports.ZiZouNotif:SendAdvanced(msg, subject, title, banner, nil, nil, true, nil, icon)
end

function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
    exports.ZiZouInteract:ShowHelpNotification(msg)
end
...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb, thetype)
	local model = modelName
	ESX.TriggerServerCallback("esx:SpawnVehicle", function(success, vehicle)
		if success then
			Wait(500)
	...
	end, model, coords, heading, thetype)
end
...
ESX.Game.GetPeds = function(onlyOtherPeds)
	local peds, myPed = {}, ESX.PlayerData.cache.playerped
...
RegisterNetEvent("esx:colorNotification")
AddEventHandler("esx:colorNotification", function(color, msg)
ESX.ColorNotification(color, msg)
end)
