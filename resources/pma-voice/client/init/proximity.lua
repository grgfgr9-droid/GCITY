...
CreateThread(function()
    ...
    local lastRadioStatus = false
    local lastTalkingStatus = false
    local voiceRefreshRate = GetConvarInt('voice_refreshRate', 200)

    while true do
        -- Attendre la reconnexion de Mumble
        while not MumbleIsConnected() do
            Wait(1000)  -- Attente plus longue avant de vérifier à nouveau
        end

        -- Vérifier la configuration de l'UI une fois et garder l'état
        if GetConvarInt('voice_enableUi', 1) == 1 then
            -- Vérification de l'état de la radio et de la parole moins fréquemment
        ...
        -- Gérer le mode spectateur en fonction de l'état du joueur
        ...
        -- Attendre une durée déterminée avant de vérifier à nouveau
        Wait(voiceRefreshRate)
    end
end)
