	...
	Citizen.CreateThread(function()
		while displaying do
			...
			if (target ~= PlayerId() and target > 0) or (GetPlayerServerId(PlayerId()) == targetId) then
				...
				if GetDistanceBetweenCoords(targetCoords, GetEntityCoords(ESX.PlayerData.cache.playerped, false), true) < 25.0 then
	...
	while true do
		...
		for k, v in pairs(activeTag) do
			...
			if (target ~= PlayerId() and target > 0) or (GetPlayerServerId(PlayerId()) == k) then
				...
				if GetDistanceBetweenCoords(targetCoords, GetEntityCoords(ESX.PlayerData.cache.playerped, false)) < 25.0 and not IsPedInAnyVehicle(targetPed, false) then
