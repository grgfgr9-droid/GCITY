...
function LeaveInstance()
	if instance.host then
		if #instance.players > 1 then
			ESX.ShowNotification("vous avez quitté l\'instance")
		end
...
AddEventHandler('instance:onInvite', function(_instance, type, data)
	...
	Citizen.CreateThread(function()
		while instanceInvite do
			...
			ESX.ShowHelpNotification("appuyez sur ~INPUT_CONTEXT~ pour entrer dans l\'instance")

			if IsControlJustReleased(0, 38) then
				EnterInstance(instanceInvite)
				ESX.ShowNotification("vous êtes entré dans l\'instance")
				instanceInvite = nil
			end
		end
	end)
	...
	Citizen.CreateThread(function()
		...
		if instanceInvite then
			ESX.ShowNotification("invitation expirée")
			instanceInvite = nil
		end
	end)
